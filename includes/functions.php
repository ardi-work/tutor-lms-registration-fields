...
function tcf_get_fields() {
    $option = get_option(TCF_OPTION_KEY, array('fields' => array()));
    return isset($option['fields']) ? $option['fields'] : array();
}
...
function tcf_create_field($field_data) {
    ...
    if (!preg_match('/^[a-z0-9_]+$/', $field_key)) {
        return new WP_Error('invalid_key', __('Field key must contain only lowercase letters, numbers, and underscores.'));
    }
    
    // Check for duplicate
    if (isset($fields[$field_key])) {
        return new WP_Error('duplicate_key', __('Field key already exists. Please use a different key.'));
    }
    
    $fields[$field_key] = array(
        'field_key' => $field_key,
        'label' => sanitize_text_field($field_data['label']),
        'meta_key' => !empty($field_data['meta_key']) ? sanitize_key($field_data['meta_key']) : $field_key,
        ...
        'display_in_profile' => !empty($field_data['display_in_profile']) ? true : false,
        'display_in_edit' => !empty($field_data['display_in_edit']) ? true : false,
        'select_options' => isset($field_data['select_options']) ? $field_data['select_options'] : '',
    );
...
function tcf_validate_required($value, $error_message) {
    if (empty(trim($value))) {
        return new WP_Error('required', !empty($error_message) ? $error_message : __('This field is required.'));
    }
    return true;
}